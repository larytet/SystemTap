...
inline int __sys_clone2(int flags, void *ustack_base, size_t ustack_size,
...
inline long  __clone(unsigned long flags, void *child_stack,
		     void *ptid, void *ctid, int tls_val)
{
...
#define FLAG_ALL (CLONE_VM|CLONE_FS|CLONE_FILES|CLONE_SIGHAND|SIGCHLD)
    ...
    //staptest// clone2 (CLONE_VM|CLONE_FS|CLONE_FILES|CLONE_SIGHAND|SIGCHLD, XXXX, 0x100000, XXXX, XXXX) = NNNN
#else
...
#ifdef __s390__
    //staptest// clone (CLONE_FS|CLONE_NEWNS, 0x1, 0x[7]?[f]+, 0x0) = -NNNN
#else
    //staptest// clone (CLONE_FS|CLONE_NEWNS, 0x1, 0x[f]+, 0x0) = -NNNN
#endif

    pid = __clone(CLONE_FS|CLONE_NEWNS, (void *)1, NULL, (void *)-1, 0);
#ifdef __s390__
    //staptest// clone (CLONE_FS|CLONE_NEWNS, 0x1, 0x0, 0x[7]?[f]+) = -NNNN
#else
    //staptest// clone (CLONE_FS|CLONE_NEWNS, 0x1, 0x0, 0x[f]+) = -NNNN
#endif
#endif
